...
public class SQLiteConnectorTest extends Activity
{
  ...
  ///* package */ void runTest() {
    ...
    try {
      mystatement = mydbc.prepareStatement("DROP TABLE IF EXISTS tt;");
    } catch (SQLException ex) {
      logUnexpectedException("prepare statement exception", ex);
      mydbc.dispose();
      return;
    ...
    try {
      mystatement = mydbc.prepareStatement("CREATE TABLE IF NOT EXISTS tt (text1 TEXT, num1 INTEGER, num2 INTEGER, real1 REAL)");
    } catch (SQLException ex) {
      logUnexpectedException("prepare statement exception", ex);
      mydbc.dispose();
      return;
    ...
    try {
      mystatement = mydbc.prepareStatement("INSERT INTO tt (text1, num1, num2, real1) VALUES (?,?,?,?)");
    ...
    try {
      mystatement = mydbc.prepareStatement("SELECT * FROM tt;");
    } catch (SQLException ex) {
      logUnexpectedException("prepare statement exception", ex);
      mydbc.dispose();
      return;
    ...
    ArrayAdapter<String> r1 = resultsAdapter; // XXX TEMP
